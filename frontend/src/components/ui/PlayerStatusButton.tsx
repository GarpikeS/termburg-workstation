...
export function PlayerStatusButton() {
  ...
  const label = authenticated ? session?.account.name.split(/\s+/)[0] : status === 'loading' ? 'Проверяем' : 'Войти';
  ...
  return (
    <button
      ...
      aria-label={authenticated ? `Открыть профиль ${session?.account.name}` : 'Войти или зарегистрироваться'}
      data-player-status
    >
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-primary/15 text-primary">
        {authenticated ? <UserRound size={18} aria-hidden="true" /> : <LogIn size={18} aria-hidden="true" />}
      </span>
