      ...
      <div className="h-full flex flex-col bg-dark-surface">
        <div className="screen-safe-header pb-4 px-5">
          <div className="flex items-center justify-between">
            <button type="button" aria-label="Назад в магазин" onClick={() => navigate('/shop')} className="min-w-11 min-h-11 flex items-center justify-center text-white/50 hover:text-primary transition-colors">
              <ArrowLeft size={20} />
            </button>
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <button type="button" onClick={() => navigate('/shop')} className="min-h-11 px-4 text-primary text-sm mt-3 underline">
            Перейти в магазин
          </button>
        </div>
      </div>
        ...
        {cartItems.map(({ productId, quantity, product }) => (
          <div key={productId} className="bg-white/5 border border-white/10 rounded-xl p-4">
            <div className="flex items-start justify-between">
              ...
              <button type="button" aria-label={`Удалить ${product!.name} из корзины`} onClick={() => removeFromCart(productId)} className="min-w-11 min-h-11 flex items-center justify-center text-white/30 hover:text-red-400 transition-colors">
                <Trash2 size={16} />
              </button>
            </div>
            <div className="flex items-center justify-between mt-3">
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="w-11 h-11 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white/60"
                >
                  <Minus size={14} />
                </button>
