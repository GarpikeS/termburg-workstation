...
const Match3GameRoute = lazy(() => import('@/components/screens/GameScreen').then(module => ({ default: module.Match3GameRoute })));
...
function AppLayout() {
  ...
  const withFreeHourGoal = isGameplayRoute(location.pathname);
  ...
  return (
    ...
    >
      <div className={`phone-frame bg-dark-surface relative flex flex-col ${standaloneSchedule ? 'phone-frame--standalone' : ''}`}>
        <main
          className={`phone-screen ${withBottomNav ? 'phone-screen--with-nav' : ''} ${withFreeHourGoal ? 'phone-screen--with-game-goal' : ''} ${standaloneSchedule ? 'phone-screen--standalone' : ''}`}
        ...
        >
          <Suspense fallback={<div className="route-loading" role="status" aria-label="Загрузка страницы" />}>
            <Routes>
              ...
              <Route path="/games/match3/levels/:bathhouseId" element={<Navigate to="/games/match3" replace />} />
              <Route path="/games/match3/play/:id" element={<Match3GameRoute />} />
